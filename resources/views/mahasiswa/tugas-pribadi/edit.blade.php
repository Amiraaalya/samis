...
@section('title', 'Edit Tugas Pribadi')
@section('page-title', 'Edit Tugas Pribadi')

@section('content')

<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form method="POST" action="{{ route('mahasiswa.tugas-pribadi.update', $tugas) }}">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400">{{ old('deskripsi', $tugas->deskripsi) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="progres"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400">
                            <option value="0"   {{ old('progres', $tugas->progres) == 0   ? 'selected' : '' }}>0%</option>
                            <option value="25"  {{ old('progres', $tugas->progres) == 25  ? 'selected' : '' }}>25%</option>
                            <option value="50"  {{ old('progres', $tugas->progres) == 50  ? 'selected' : '' }}>50%</option>
                            <option value="75"  {{ old('progres', $tugas->progres) == 75  ? 'selected' : '' }}>75%</option>
                            <option value="100" {{ old('progres', $tugas->progres) == 100 ? 'selected' : '' }}>100%</option>
                        </select>
                    </div>
                </div>
            </div>
